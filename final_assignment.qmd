...
```{r}
ggplot(player_statistics, aes(ranking, win_rate))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE)+
  labs(title = "Association between players ranking and their win rate", y = " Ratio of games won", x= "Player's rank")+
  theme_minimal()
```
...
```{r}
ggplot(player_statistics, aes(ranking, games_nr,color = win_rate))+
  geom_point()+
  labs(title = "Relationship between the experiance and the ranking of the player", y= "Player ranking", x="Total number of games played", color = "Win rate")+
  theme_bw()
```

### Model building

#### 1st model: Ranking predicted by total nr of games, win ratio and the average turns, and the white role ratio

```{r}

games_lm <- lm(ranking ~ win_rate + games_nr, data = player_statistics)
...
Results: VIF shows a lower value, however the plot indicates collienarity for the total number of games, also there is a significant Spearman correlation between ranking and total number of games
...
Conclusion: total nr of games may be showing (plot vs vif test) a VIF (collinearity) too high to be included in the model, also seem to have influential outliers (std.residuals vs Cook's distance)
...
Results: Homogenity, linearity and normailty seem acceptable; small protursion inn predicitons
...
Conclusion: The influential observations and the predictions might be questionable based on the curves and std.residuals above, although the Cook's distances seem acceptable; other assumptions seem acceptable
...
Conclusion: The influential observations and the predictions might be questionable based on the std.residuals, although the Cook's distances seem acceptable, other assumptions seem acceptable
...
Unfortunately the total number of games could not be included in the models, thus conclusion cannot be drawn regarding that predictor. Also the std. residuals of all three models may seem questionable.

Regarding the coefficients, the win rates seemed significant predictors of the ranking of the player as shown in the coefficient table of the 2nd model above. The rate of the playing the initiating (white) role appeared no t to contribute significantly to the 3rd model as per the table.

Based on the results of the ANOVA and the Likelihood ratio test the difference between the models (with or without the role variable) has not reached the significance level, neither has the AIC dropped in a considerable manner. The adjusted R squares in both models are quite low, leaving a great proportion of unexplained variance.

All together, even though the models show some significant results, the models do not explain too much of the variance. Additionally model diagnostics and possible violation of assumptions might leave the results unreliable.
